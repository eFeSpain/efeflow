...
/**
 * The state this packet is in as far as the chain now being walked can see.
 *
 * Conntrack is itself a hook, at priority -200 on prerouting and output, and a
 * chain earlier than that runs before the packet has an entry at all — so
 * `ct state` there matches nothing but `invalid`, whatever the connection
 * turns out to be a moment later. This was evaluated the same at every
 * priority, so a `raw` chain reading `ct state invalid counter drop` — the
 * ordinary first line of one — passed every packet here and dropped every
 * packet on the machine.
 *
 * Loopback hides it, which is why it took a real arrival to find: a packet
 * this host generated has already been through conntrack on the way out, so
 * by the time prerouting sees it the entry exists.
 */
const ctState = p => p.ctReady === false ? "invalid"
                   : p.tracked ? p.state : "untracked";
...
const MATCHERS = [
  ...
  { re: /\bct\s+state\s+(?!vmap\b)(!=\s*)?(\{[^}]*\}|[\w,]+)/,
    ok: (m,p) => negated(m[1], states(m[2]).includes(ctState(p))) },
  ...
  { re: /\bct\s+status\s+(!=\s*)?(snat|dnat)\b/,
    ok: (m,p) => negated(m[1], p.ctReady !== false
                             && !!(p.tracked && (m[2] === "dnat" ? p.dnat : p.snat))) },
